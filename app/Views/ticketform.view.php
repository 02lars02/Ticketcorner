...
<form action="<?= $isEdit ? 'editTicket' : 'addTicket' ?>" method="post">
    ...
    <select id="bonus" name="bonus" id="bonus" class="form-control">
        <?php foreach ($bons as $oneBon): ?>
            <option value="<?= $oneBon->id ?>" <?php if(isset($ticketBuy) && $oneBon->id == $ticketBuy->bonus->id) { ?> selected <?php }?>><?= $oneBon->text ?></option>
        <?php endforeach; ?>
    </select><br>
